...
import { Anchor, Avatar, Box, Button, Divider, Flex, Group, HoverCard, Stack, Text, TextInput, Title, Tooltip } from '@mantine/core';
...
export const MultipleContainers = ({
		...
		(args) => {
			...
			if (overId != null) {
				if (items.hasOwnProperty(overId)) {
					const containerItems = items[overId].map((item: { value: string; }) => item.value);
	...
	const getIndex = (id: UniqueIdentifier) => {
		...
		const index = items[container].findIndex((item: { value: UniqueIdentifier; }) => item.value === id);
